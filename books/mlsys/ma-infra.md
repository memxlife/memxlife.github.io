...
> *This article merges two earlier drafts into a single, self-consistent, long-form chapter. Its goal is not merely to describe Claude Code as a product, but to use it as a serious teaching example for computer science students who want to understand what a modern AI coding agent actually is. The central claim is simple: a production coding agent is not mainly a chatbot with autocomplete and a shell. It is a runtime architecture that wraps a probabilistic model in deterministic systems scaffolding.*

For a long time, students were taught to think about AI coding tools in a very narrow way. You type a question, the model suggests a patch, perhaps it explains an error message, and the interaction stops there. That mental model belonged to an earlier stage of the field. It was not wrong, but it was incomplete. It treated the language model as the whole product and everything around it as a convenience layer. Once one studies a system like Claude Code carefully, that interpretation becomes too small to be useful.
